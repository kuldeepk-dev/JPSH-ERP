...
if (! function_exists('generalBoards')) {
    function generalBoards(): array
    {
        ...
        if (is_string($boards)) {
            $decodedBoards = json_decode($boards, true);

            if (json_last_error() === JSON_ERROR_NONE && is_array($decodedBoards)) {
        ...
        return array_values(array_filter(array_map(static function ($board) {
            return trim((string) $board);
        }, $boards)));
    }
}

if (! function_exists('selectedBoard')) {
